...
EYE_AR_THRESH = 0.29
CONSECUTIVE_ALARM_SENS = 1800 	# Milliseconds for consecutive alarm
DROWSINESS_ALARM_SENS = 6000 	# Milliseconds for general alarm, will decrement by 1 when open
TILT_ALARM_SENS = 3000 		# Milliseconds for tilt alarm
...
flag = 0 		# For prolonwged eye closure
long_flag = 0 	# For eye closure over time (non-consecutive)
tilt_flag = 0 	# For tilt
last_frame_time = 0 	# For timing
curr_frame_time = 0 	# For timing
diff = 0 # For timing
...
while True:
	...
	for face in faces:
		...
		if (frames_run < 30):
			...
			if (frames_run == 5):
				...
				cv2.putText(frame, "CLOSE eyes", (270, 300),
					cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 0, 255), 2)
				ear_closed += ear
		...
		curr_frame_time = datetime.datetime.now()
		difference = curr_frame_time - last_frame_time
		time_diff = difference.microseconds/1000 + difference.seconds*1000
		# If the head is tilted more than 20 degrees, 
		if (tilt < -20 or tilt > 20):
			tilt_flag += time_diff
		# If eye is safe
		else:
			# Reset tilt flag
		...
		if (ear < dynamic_ear_thresh):
			flag += time_diff
			long_flag += time_diff
			print ("FLAG: ", flag, " LONGFLAG: ", long_flag, "TILT: ", tilt)
		# If eye is safe
		else:
			# Reset flag
			flag = 0
		...
		if (flag > CONSECUTIVE_ALARM_SENS or long_flag > DROWSINESS_ALARM_SENS or tilt_flag > TILT_ALARM_SENS):
			alarmOn() 
	...
	cv2.imshow("Eye of Gatsby", frame) # Disable for production
